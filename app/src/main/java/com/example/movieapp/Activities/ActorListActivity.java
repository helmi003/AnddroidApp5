...
import com.example.movieapp.adaptater.ActorAdapter;
...
public class ActorListActivity extends BaseActivity implements ActorAdapter.OnItemClickListener {
    ...
    private void fetchActorData() {
        ...
        new Thread(() -> {
            actorList.clear();
            actorList.addAll(database.actorDao().getAllActors()); // Get all actors from the database
    ...
    private void deleteActor(Actor actor) {
        ...
        new Thread(() -> {
            database.actorDao().delete(actor);
